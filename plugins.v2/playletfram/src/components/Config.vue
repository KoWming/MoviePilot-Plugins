                  ...
                  <v-row>
                    <v-col cols="12" md="6">
                      <v-text-field
                        ...
                        :min="-20"
                        :max="20"
                        :rules="[v => v === null || v === '' || (Number(v) >= -20 && Number(v) <= 20) || '百分比必须在-20到20之间']"
                        hint="自动出售利润率区间下限。"
                        persistent-hint
...
const loadConfig = async () => {
  try {
    ...
    if (res) {
      ...
      config.auto_sell_threshold_max = res.auto_sell_threshold_max !== undefined ? res.auto_sell_threshold_max : 20;
...
watch(() => props.initialConfig, (newConfig) => {
  if (newConfig && Object.keys(newConfig).length > 0) {
    ...
    config.auto_sell_threshold_max = newConfig.auto_sell_threshold_max !== undefined ? newConfig.auto_sell_threshold_max : 20;
