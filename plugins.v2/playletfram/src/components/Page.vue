  ...
  <div class="magicfram-page">
    ...
    <v-dialog v-model="showSellDialog" max-width="400">
      <v-card>
        ...
        <v-card-text class="pa-4">
          ...
          <div v-else class="text-caption text-grey mt-3">暂无成本或当前价格数据，无法计算预计盈亏。</div>
    ...
    <v-card flat class="rounded border">
      <v-card-title class="text-subtitle-1 d-flex align-center px-3 py-2 bg-gradient-primary">
        ...
        <span class="text-white">开心农场</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text class="px-3 py-3" style="position: relative;">
            ...
            <v-card flat class="rounded border flex-grow-1 d-flex flex-column">
              <v-card-title class="text-subtitle-2 px-3 py-2 bg-blue-lighten-5 d-flex align-center">
                ...
                <v-chip color="blue-grey-lighten-4" size="small" class="px-3 text-grey-darken-3" variant="flat">
                  <v-icon icon="mdi-auto-fix" size="small" class="mr-1 magic-anim" color="purple"></v-icon>
            ...
            <v-card flat class="rounded mb-3 border">
              <v-card-title class="text-subtitle-2 px-3 py-2 bg-green-lighten-5 d-flex align-center">
                ...
                <div class="d-flex align-center overflow-hidden flex-grow-1 mr-2">
                  ...
                  <span class="text-caption text-grey ml-2 text-truncate" style="font-size: 0.7rem;">{{ cropSubtitle }}</span>
                </div>
                <v-btn
        ...
        <v-card flat class="rounded border flex-grow-1">
          <v-card-title class="text-subtitle-2 px-3 py-2 bg-brown-lighten-5 d-flex align-center">
            ...
            <div class="d-flex align-center overflow-hidden flex-grow-1 mr-2">
              ...
              <span class="text-caption text-grey ml-2 text-truncate" style="font-size: 0.7rem;">{{ animalSubtitle }}</span>
            </div>
            <v-btn
          ...
          <v-card-text class="px-3 py-2">
            <div v-if="warehouse.length > 0">
              <v-table density="compact">
                ...
                <tbody>
                  <tr v-for="(item, i) in sortedWarehouse" :key="'warehouse-'+i">
                    <td>{{ item.name }}</td>
        ...
        <v-card flat class="rounded mb-3 border">
          <v-card-title class="text-subtitle-2 px-3 py-2 bg-light-green-lighten-5">
            ...
            菜市场
            <span class="text-caption text-grey ml-2" style="font-size: 0.7rem;">市场价格每24小时更新一次，波动范围±20%</span>
          </v-card-title>
          <v-card-text class="px-3 py-2">
                ...
                <v-table density="compact">
                  ...
                  <tbody>
                    <tr v-for="(item, i) in market.filter(m => m.type === 'crop')" :key="'market-crop-'+i">
                      ...
                      <td class="text-center">
                        <template v-if="parseFloat(item.last_price || 0) > 0">
                          <span :class="(parseFloat(item.price) - parseFloat(item.last_price)) > 0 ? 'text-red' : ((parseFloat(item.price) - parseFloat(item.last_price)) < 0 ? 'text-green' : 'text-grey')">
                            {{ (parseFloat(item.price) - parseFloat(item.last_price)) > 0 ? '+' : '' }}{{ ((parseFloat(item.price) - parseFloat(item.last_price)) / parseFloat(item.last_price) * 100).toFixed(2) }}%
                          </span>
                        </template>
...
const market = ref([]);
const cropSubtitle = ref("");
const animalSubtitle = ref("");
...
const refreshData = async () => {
  ...
  try {
    ...
    if (res && res.success && res.farm_status) {
      ...
      cropSubtitle.value = data.crop_subtitle || "";
      animalSubtitle.value = data.animal_subtitle || "";
...
// 仓库排序
const sortOrder = ref('asc'); // default, asc, desc
...
@keyframes magic-pulse {
  0% { transform: scale(1) rotate(0deg); filter: drop-shadow(0 0 2px rgba(156, 39, 176, 0.4)); }
  50% { transform: scale(1.2) rotate(15deg); filter: drop-shadow(0 0 5px rgba(156, 39, 176, 0.8)); }
  100% { transform: scale(1) rotate(0deg); filter: drop-shadow(0 0 2px rgba(156, 39, 176, 0.4)); }
}
