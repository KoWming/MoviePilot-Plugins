  ...
  <div class="plugin-config">
    <v-card flat class="rounded border">
      ...
      <v-card-title class="text-subtitle-1 d-flex align-center px-3 py-2 bg-gradient-primary">
        ...
        <span class="text-white">拾刻农场配置</span>
        <v-spacer />
                    ...
                    <div class="setting-content flex-grow-1">
                      <div class="d-flex align-center justify-space-between w-100">
                        <div class="setting-text">
                          ...
                          <div class="text-caption text-grey">是否启用拾刻农场插件</div>
                        </div>
              ...
              <v-row>
                <v-col cols="12" md="6">
                  <v-text-field
                    ...
                    type="password"
                    variant="outlined"
                    hint="登录拾刻站点后，从浏览器开发者工具中复制 Cookie"
                    persistent-hint
                      ...
                      <div class="tooltip-container">
                        ...
                        <div class="custom-tooltip">
                          <div class="custom-tooltip-content">
                            <span class="tooltip-text">使用已添加站点的 Cookie</span>
                          </div>
                        </div>
                      </div>
                    </template>
  ...
  try {
    ...
    if (res && res.success) {
      if (typeof res.cookie === 'string' && res.cookie.trim().toLowerCase() === 'cookie') {
        showNotification('站点 Cookie 无效，请在站点管理中配置真实 Cookie', 'error');
        return;
      }
      config.cookie = res.cookie;
      showNotification('已成功获取站点 Cookie', 'success');
    } else {
      showNotification(res.msg || '获取站点 Cookie 失败', 'error');
    }
  } catch (e) {
    console.error('获取站点 Cookie 失败:', e);
    showNotification('获取站点 Cookie 失败: ' + (e.message || '未知错误'), 'error');
  } finally {
