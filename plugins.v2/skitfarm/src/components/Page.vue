  ...
  <div class="skitfarm-page">
    ...
    <v-card flat class="rounded border">
      <v-card-title class="text-subtitle-1 d-flex align-center px-3 py-2 bg-gradient-primary">
        <v-icon icon="mdi-star-four-points" class="mr-2" color="white" size="small"></v-icon>
        <span class="text-white">拾刻农场</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text class="px-3 py-3" style="position: relative;">
        <div style="position: absolute; top: 0; left: 0; right: 0; z-index: 50; padding: 0 10px 10px 10px;">
          <transition name="slide-fade">
            <v-alert
              ...
              type="success"
              density="compact"
              class="mb-2 text-caption skit-alert"
              variant="elevated"
              closable
                ...
                <v-row dense align="center">
                  ...
                  <v-col cols="5">
                    ...
                    <div class="d-flex align-center justify-center mt-1">
                      ...
                      <span class="text-body-2" v-if="pluginStatus.cron">
                        {{ pluginStatus.next_run_time || '等待调度' }}
                      </span>
            ...
            <v-card flat class="rounded border flex-grow-1 d-flex flex-column">
              <v-card-title class="text-subtitle-2 px-3 py-2 bg-blue-lighten-5 d-flex align-center">
                ...
                <v-chip color="blue-grey-lighten-4" size="small" class="px-3 text-grey-darken-3" variant="flat">
                  <v-icon icon="mdi-star-four-points" size="small" class="mr-1 fire-anim" color="deep-orange"></v-icon>
        ...
        <!-- 菜市场 -->
        <v-card flat class="rounded mb-3 border">
          <v-card-title class="text-subtitle-2 px-3 py-2 bg-light-green-lighten-5 d-flex align-center flex-wrap">
            <v-icon icon="mdi-store" class="mr-2" color="light-green" size="small"></v-icon>
            <div class="d-flex align-center flex-wrap flex-grow-1 min-width-0">
              <span>菜市场</span>
              <span class="text-caption text-grey ml-2" style="font-size: 0.7rem;">每日0、4、8、12、16、20点刷新 | 波动范围±20%</span>
            </div>
            <span v-if="marketNextRefresh" class="text-caption text-blue-grey-darken-1 ml-auto market-next-refresh-inline text-right">
              {{ marketNextRefresh }}
            </span>
          </v-card-title>
          <v-card-text class="px-3 py-2">
...
const refreshData = async () => {
  ...
  try {
    ...
    if (res && res.success && res.farm_status) {
      ...
      marketNextRefresh.value = data.market_next_refresh || '';
...
onMounted(async () => {
  ...
  try {
    ...
    if (res) {
      ...
      pluginStatus.next_run_time = res.next_run_time || '';
      pluginStatus.time_until_next = res.time_until_next || '';
      pluginStatus.task_status = res.task_status || '';
...
.bg-gradient-primary {
  background: #ff85a2;
  box-shadow: 0 2px 8px rgba(255, 133, 162, 0.35);
}
...
.skit-alert {
  ...
  color: #8a3d56 !important;
  border: 1px solid #ffc1d1 !important;
}

.skit-alert :deep(.v-alert__overlay),
.skit-alert :deep(.v-alert__underlay) {
  opacity: 0 !important;
  background: transparent !important;
}

.skit-alert :deep(.v-alert__prepend .v-icon),
.skit-alert :deep(.v-alert__close .v-btn),
.skit-alert :deep(.v-alert__close .v-icon) {
